      ...
      <div v-if="isPolling || taskStatus === 'FAILURE'" class="progress-section">
        ...
        <div class="status-display">
          ...
          <div class="status-text">
            ...
            <p v-if="taskStatus === 'ANALYZING'">正在生成 PCA、UMAP 及通路热图...</p>
        ...
        <el-tabs v-model="activeTab" class="demo-tabs">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-card shadow="hover">
                  <template #header>UMAP - By Cluster</template>
                  <el-image
          ...
          <el-tab-pane label="差异分析 (Differential)" name="diff">
             <el-row :gutter="20">
              <el-col :span="12">
                <el-card shadow="hover">
                  <template #header>Volcano Plot (Cluster 0 vs 1)</template>
                  <el-image
                    :src="getImageUrl(currentJobId, resultData.images.volcano)"
                    :preview-src-list="[getImageUrl(currentJobId, resultData.images.volcano)]"
                    fit="contain"
                  />
                </el-card>
              </el-col>
...
import { startAnalysis, getTaskStatus, getImageUrl, API_BASE_URL } from '@/api/scfea';
...
const startPolling = (jobId: string) => {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      // 网络错误等情况，不一定马上停止，可以容错几次，这里简单处理直接停止
      clearInterval(timer);
      isPolling.value = false;
...
const downloadFile = (fileKey: string) => {
  ...
  // files[fileKey] 是服务器上的绝对路径，通过 API_BASE_URL 访问静态资源
  // 注意：后端返回的是绝对路径，静态资源挂载的是相对路径
  // 我们需要拼接正确的下载链接。
  // 在 scfea_analysis.py 里，如果返回的是绝对路径，前端下载稍微麻烦点。
  // 简单起见，我们直接构造成 http://localhost:8000/results/{job_id}/{filename}

  const originalPath = resultData.value.files[fileKey];
  const filename = originalPath.split(/[\\/]/).pop(); // 提取文件名
